...
const LoadingSkeleton: React.FC<ContainerProps> = ({
  ...
  const skeletonIcon =
    (error && alertOutline) ||
    (loading && cloudDownloadOutline) ||
    informationOutline

  const skeletonTitle = title || (error ? 'Error' : 'Loading')

  const skeletonMessage =
    message ||
    (error ? 'An error was encountered while loading this data!' : 'Loading')
    ...
    <div className={`loading-skeleton ${isInline ? 'is-inline' : ''}`}>
      <div className="loading-skeleton-card">
        ...
        <div className="loading-skeleton-card-text-container">
          <div className="loading-skeleton-card-title">{skeletonTitle}</div>
