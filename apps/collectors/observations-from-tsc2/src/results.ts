...
/** Controls how TSC Lua snapshots are converted into observations. */
    ...
    luaFiles.map(async (code) => {
      ...
      try {
        lua.global.set("console_log", (...args: never[]) => {
          logger.debug(`[lua] ${JSON.stringify(args)}`);
        });
        ...
        const data = lua.global.get(targetTable) as Record<number, string>;

        return { platform, data, date };
      } finally {
        lua.global.close();
      }
    }),
...
const itemLookupCache = new Map<number, ReturnType<typeof findItemByGameId>>();

const findCachedItemByGameId = (id: number) => {
  const cached = itemLookupCache.get(id);
  if (cached) return cached;
  const pending = findItemByGameId(id);
  itemLookupCache.set(id, pending);
  return pending;
};
...
const _dataStringToObservations = async (
  ...
  const item = await findCachedItemByGameId(id);
  if (!item) return [];
                  ...
                  try {
                    return await _dataStringToObservations(
                      ...
                      options?.observationDate
                        ? Date.parse(`${options.observationDate}T00:00:00Z`)
                        : parsed.date!,
                    );
                  } catch (e) {
                    if (process.env.CI) {
