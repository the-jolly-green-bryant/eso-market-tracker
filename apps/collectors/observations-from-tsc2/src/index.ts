...
const findAddon = (value: unknown): Record<string, unknown> | undefined => {
  if (Array.isArray(value)) {
    ...
    if (Object.values(record).some((entry) => entry === ADDON_ID)) return record
...
export const getRemoteAddonVersion = async () => {
  ...
  try {
    ...
    const addon = findAddon(JSON.parse(await fsPromises.readFile(output, 'utf8')))
...
export const collectObservations = async (options?: { maxWrites?: number }) => {
  ...
  if (
    previousSource?.contentSha256 === contentSha256 &&
    !options?.maxWrites
  ) {
