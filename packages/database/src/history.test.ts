...
import { readItemPricingArchive } from "./archives";
...
describe("item history projections", () => {
  ...
  it("merges, deduplicates, and sorts an archived projection", async () => {
    ...
    const archive = await readItemPricingArchive(item.id, { repositoryRoot });
    expect(archive[path.basename(historicalPath)]).toEqual([
